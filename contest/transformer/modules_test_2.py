...
class LossComputeBase(nn.Module):
    ...
    def forward(self, x, target):
        loss = F.cross_entropy(x, target, ignore_index=0, reduction='sum')
        return loss
...
class EncoderCNN(nn.Module):
    def __init__(self, d_model, d_ff, n_head, num_encoder_layers, dropout=0.1):
        ...
        self.src_embed = nn.Sequential(nn.Conv1d(1,d_model, 1), Residual(d_model,d_model), 
    ...
    def forward(self, signal):
        ...
        src_mask = torch.zeros(enc_output.size(0),1,enc_output.size(1),dtype=torch.bool).to(enc_output.device)
        #print(src_mask.size())
...
class Decoder(nn.Module):
    def __init__(self, d_model, d_ff, n_head, label_vocab_size, d_word_vec, num_decoder_layers,
        ...
        self.d_model = d_model
        self.label_word_embed = nn.Embedding(label_vocab_size, d_word_vec, padding_idx=0)
    ...
    def forward(self, label_seq, enc_out, src_mask):
        ...
        tgt_mask = make_std_mask(label_seq, 0)
...
class Transformer(nn.Module):
    def __init__(self, d_model, d_ff, n_head, num_encoder_layers,
        ...
        self.encoder = EncoderCNN(d_model, d_ff, n_head, num_encoder_layers, dropout)
    ...
    def forward(self, signal, label):
        ...
        seq_logit = self.final_proj(dec_out)
        return seq_logit
